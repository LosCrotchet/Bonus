...
| `pointer_offset` | 在自动锚点基础上追加的像素偏移；正 X 向右、正 Y 向下，用于避开目标或对话框重叠 |
...
| `minimum_display_time` | 允许点击继续前的最短时间；它和逐字显示都完成后，右下角三角提示才会渐显 |
...
- 正文按约 8 字/秒逐字显示，并约每两个字符播放一次轻微变调的打字音。文字未完成时首次点击只会立即补全文字，不会进入下一步；三角提示出现后的下一次点击才会推进。
