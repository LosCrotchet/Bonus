...
func _run_test() -> void:
	...
	assert(is_equal_approx(float(settings_constants["DEFAULT_MASTER_VOLUME"]), 0.7))
	assert(is_equal_approx(float(settings_constants["DEFAULT_SFX_VOLUME"]), 1.0))
	assert(is_equal_approx(float(settings_constants["DEFAULT_MUSIC_VOLUME"]), 0.5))
	assert(settings_constants["DEFAULT_RESOLUTION"] == Vector2i(1280, 720))
	assert(int(settings_constants["DEFAULT_WINDOW_MODE"]) == SettingsService.WindowMode.WINDOWED)
	assert(str(settings_constants["DEFAULT_LOCALE"]) == "zh_CN")
	assert(int(settings_constants["DEFAULT_GAME_SPEED"]) == SettingsService.GameSpeed.SLOW)
	assert(bool(settings_constants["DEFAULT_SHOW_STATUS_TEXT"]))
	assert(not bool(settings_constants["DEFAULT_DOUBLE_CLICK_ACTIONS"]))
	assert(not bool(settings_constants["DEFAULT_USE_SIMPLIFIED_CARDS"]))
