...
const SETTINGS_POSITION = Vector2(1085, 450)
...
func update_state(to_state:int):
	...
	match now_state:
		...
		7:
			...
			settings_tween.tween_property($SettingsMenu, "position:x", SETTINGS_POSITION.x+500, WAIT_TIME)
			var tmp = func():
				settings_tween = null
	...
	match to_state:
		...
		7:
			...
			$SettingsMenu.position.x = SETTINGS_POSITION.x+500
			settings_tween = get_tree().create_tween().set_trans(Tween.TRANS_CUBIC).set_parallel()
			settings_tween.tween_property($SettingsMenu, "modulate", Color(1, 1, 1, 1), WAIT_TIME)
			settings_tween.tween_property($SettingsMenu, "position:x", SETTINGS_POSITION.x, WAIT_TIME)
			var tmp = func():
				settings_tween = null
			settings_tween.tween_callback(tmp).set_delay(WAIT_TIME)
	now_state = to_state
