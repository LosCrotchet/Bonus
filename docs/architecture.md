...
`app/app.tscn` 最终作为程序主场景，负责装载功能场景和放置全局过渡层，不保存具体牌局数据。

### 主菜单与设置

- `features/main_menu/` 负责开始游戏、进入设置和退出。
- `features/settings/` 负责音量、窗口模式和分辨率等配置。
- 设置持久化由独立 Autoload 服务完成，界面只编辑服务暴露的配置。
...
- `tests/integration/`：从开始游戏到产生胜者的完整牌局、AI 接入和状态序列化。
