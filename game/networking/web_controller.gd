...
func load_game(game_scene_path):
	...
	is_gaming = true
	var corrected_path = game_scene_path.replace("res://scene/", "res://scenes/")
	get_tree().change_scene_to_file(corrected_path)
...
func player_loaded():
	...
	if multiplayer.is_server():
		...
		if players_loaded == players.size():
			print("All ready!")
			load_game.rpc("res://scenes/game/game_scene.tscn")
			players_loaded = 0
