...
func find_type(hands, dice_result, type_to_find:int = -1, last_played:Array = []):
	...
	if dice_result == -1 and last_played != []:
		dice_result = len(last_played)
	var result = []
	if dice_result == 1:
		if type_to_find == 0 or type_to_find == -1:
			for item in hands:
	...
	if dice_result == 3:
		...
		if type_to_find == 1 or type_to_find == -1:
			for i in range(len(hands)-2):
	...
	if dice_result == 4:
			...
			for i in range(len(hands)-3):
				if hands[i].y == hands[i+1].y-1 and hands[i+1].y == hands[i+2].y-1 and \
				   ...
				   (len(last_played) == 0 or hands[i].y > last_played[0].y):
					result.append([hands[i], hands[i+1], hands[i+2], hands[i+3]])
		if type_to_find == 2 or type_to_find == -1:
			var cmp_flag
		...
		if type_to_find == 3 or type_to_find == -1:
			for i in range(len(hands)-3):
	...
	if dice_result == 6:
		...
		if type_to_find == 4 or type_to_find == -1:
			for i in range(len(hands)-2):
		...
		if type_to_find == 5 or type_to_find == -1:
			for i in range(len(hands)-5):
				if hands[i].y == hands[i+1].y and hands[i+2].y == hands[i+3].y and\
