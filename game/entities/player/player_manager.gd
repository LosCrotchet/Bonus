	...
	for cnt in range(17):
		for i in range(player_count):
			if DeckManager.GameMode != 2:
				...
				Players[i].add_card(result, 2*DeckManager.WAIT_TIME)
			#await get_tree().create_timer(0.2).timeout
		await get_tree().create_timer(DeckManager.WAIT_TIME).timeout
...
func _on_game_manager_draw_card(id, num):
	...
	for i in range(num):
		...
		Players[id].add_card(result, 2*DeckManager.WAIT_TIME)
		play_sound(1)
	...
	if DeckManager.GameMode == 2:
		for i in range(player_count):
			if Players[i].order == order:
				#add_card_to(card, i)
				Players[i].add_card(card, 2*DeckManager.WAIT_TIME)
				break
